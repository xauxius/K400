...
public class InstanceCombiner : MonoBehaviour
{
    ...
    public MeshFilter[] maist;

	[ContextMenu("Add Meshes")]
	private void AddMesh()
    {
		maist = GetComponentsInChildren<MeshFilter>();

		foreach (MeshFilter maist in maist)
        {
            if (listMeshFilter.Count <1500) {
                listMeshFilter.Add(maist.GetComponent<MeshFilter>());
            }
        }

    }

	[ContextMenu("Remove Meshes")]
